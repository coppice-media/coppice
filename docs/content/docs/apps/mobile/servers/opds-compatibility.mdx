...
Excluding Coppice itself, the following servers have been tested:
...
<Callout type="info">
	This only applies to OPDS servers that support progression tracking. If you are using a Coppice
	server, it does not apply since Coppice uses its own custom API for progression syncing.
</Callout>
