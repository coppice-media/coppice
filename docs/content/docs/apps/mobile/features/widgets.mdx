...
Coppice has limited support for [widgets](https://support.apple.com/en-us/118610) on iOS devices. There are definitely some quirks with the current implementation, so be sure to read the behaviors and limitations for each supported widget type below.
...
- This widget is updated when entering any Coppice server or the local library tab
