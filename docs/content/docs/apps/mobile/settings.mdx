...
You do not need a Coppice server to use the app. Any OPDS-compatible server will work for basic catalog browsing and reading.

When disabled, the app simply hides any Coppice-specific features:

- Only OPDS servers may be configured
- Only OPDS servers are shown during server selection

A few important notes when Coppice features are disabled:

- Existing Coppice servers will still be stored, but hidden from view
- You won't get progress syncing, since that relies on Coppice-specific APIs
