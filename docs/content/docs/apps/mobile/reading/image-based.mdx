...
Any format that is image-based will generally use the same image-based reader. This includes formats like `.cbz`/`.zip` and `.cbr`/`.rar` files that contain images of the pages. PDF files are considered here as well, at least from Coppice server sources.
