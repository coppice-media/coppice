...
The EPUB reader in Coppice's mobile app is powered by Readium, providing a native reading experience for digital books. While still being refined, it offers essential reading functionality with customizable themes and settings.
...
Your reading progress is automatically saved and synced across devices when connected to a Coppice server.

Like the image-based reader, the EPUB reader also supports:

- **Incognito Mode**: Read without saving progress or location data
- **Reading Timer**: Track time spent reading (with sync support for Coppice servers)
