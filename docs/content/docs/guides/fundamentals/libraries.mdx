...
There are a couple key concepts to go over regarding how Coppice represents libraries:
...
As long as a directory is accessible by Coppice, and abides by the above specification, you can create a library with it. This allows for convenient configurations like network drives, etc.

## Supported Patterns

There are way too many varying organizational preferences for Coppice to support all of them. Instead, Coppice supports two different patterns for organizing your library:
...
A few examples to help illustrate the difference between the two library patterns and how Coppice will interpret their differing structures.
...
When Coppice scans this library as a collection-priority library, it will create 3 series:

1. _Shannon, Samantha_
2. _Sanderson, Brandon_
3. _J.R.R. Tolkien_.

When Coppice scans this library as a series-priority library, it will create 4 series:
...
Below is a visualization of this library in Coppice, using a file tree for simplicity:
...
When Coppice scans this library as a series-priority library, it will create 2 series:
...
When Coppice scans this library as a series-priority library, it will create 7 series:
...
Unfortunately, Coppice does not support any other patterns, as there are simply too many ways to organize a library to support them all. If you have a library that doesn't quite fit either of these patterns, or you just prefer a different organization method, you can always use the [File Explorer](/docs/guides/features/file-explorer) to navigate your library. This is akin to using a native file explorer to navigate your filesystem.
...
> Deleting a library will remove all of the series, books, and other entities associated with the books (e.g., read progress, reading lists, etc) contained within it. Your actual files will not be deleted, but they will no longer exist in Coppice.
>
