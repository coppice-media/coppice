...
- accepting reading-state updates from Kobo (Stump emits a reading-state projection during library sync, derived from its own reading sessions; see [`KoboSync::sync_items`](https://github.com/stumpapp/stump/blob/nightly/apps/server/src/routers/kobo/sync.rs#L243-L286) and [`BookEntitlementContainer::from_media`](https://github.com/stumpapp/stump/blob/nightly/core/src/kobo/entity.rs#L363-L426))
