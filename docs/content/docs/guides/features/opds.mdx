...
So long as an OPDS client _properly_ implements the OPDS specification, you should be able to use it with Coppice. Each OPDS client might have a different way of collecting this information during their onboarding steps. If you have any trouble using your preferred client, please open an issue on [GitHub](https://github.com/stumpapp/stump/issues/new/choose) or feel free to pop into the [Discord](https://discord.gg/63Ybb7J3as) for help. This page can be updated with your findings to help others in the future.

## Supported OPDS Versions

Coppice fully supports OPDS 1.2 and has experimental support for 2.0. At the time of writing, there are few clients which actually support 2.0, so it is difficult to test. If you have any experiences with OPDS 2.0, please consider updating this page with your findings!

### OPDS 1.2

The general structure of the URL to connect to your Coppice server is:

`http(s)://your-server(:10801)(/baseUrl)/opds/v1.2/catalog`

Where `your-server` is the domain or IP address of your Coppice server, `:10801` is the port (if you are using the default port), and `/baseUrl` if you've put Coppice behind a some sort of proxy that includes a path to access it. A few examples just to hopefully help clarify:

- `http://192.168.1.1:10801/opds/v1.2/catalog`
- `https://coppice.example.com/opds/v1.2/catalog`

#### Tested Clients

> **Note:** The ✨ emoji indicates a client which the developer of Coppice personally uses

The following clients have been tested with Coppice:

| OS            |                                      Application                                       | Page Streaming |                                                                     Issues/Notes |
| ------------- | :------------------------------------------------------------------------------------: | -------------: | -------------------------------------------------------------------------------: |
| iOS & Android |                               [Coppice](/docs/apps/mobile)                               |             ✅ |                                                                               ✨ |
...
- `https://coppice.example.com/opds/v2.0/catalog`

<Callout type="info">
	Coppice hasn't implemented `facets` support yet, but it is planned for a future release
</Callout>
