...
When uploading is not enabled, the upload routes **will not be attached to your server**, and the Editor will not show the upload interface. Attempting to upload directly through the API will result in HTTP Error 404.
...
Coppice attempts to validate uploads in order to protect the host system from misuse of upload functionality, but it should be used with care. The following section describes the validation Coppice applies to incoming upload requests.
