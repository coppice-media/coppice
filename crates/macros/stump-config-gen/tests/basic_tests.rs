...
mod env_keys {
	...
	pub const NESTED_PORT_ENV_KEY: &str = "NESTED_PORT_ENV_KEY";
	pub const NESTED_LIST_OF_NAMES_KEY: &str = "NESTED_LIST_OF_NAMES_KEY";
}
use env_keys::*;
...
struct NestedGroup {
	#[default_value(3000)]
	#[debug_value(4000)]
	#[env_key(NESTED_PORT_ENV_KEY)]
	pub port: u32,

	#[default_value(vec![])]
	#[env_key(NESTED_LIST_OF_NAMES_KEY)]
...
#[test]
fn test_nested_group_new_and_debug() {
	assert_eq!(
		NestedConfig::new(),
		NestedConfig {
			group: NestedGroup {
	...
	assert_eq!(NestedConfig::debug().group.port, 4000);
}

#[test]
fn test_nested_group_from_environment() {
	temp_env::with_vars(
		[
			(NESTED_PORT_ENV_KEY, Some("1337")),
			(NESTED_LIST_OF_NAMES_KEY, Some("Alice,Bob")),
			(MAYBE_BOO_KEY, Some("Boo")),
		],
		|| {
...
#[test]
fn test_nested_group_from_flat_toml() {
	// The fixture uses flat keys; `#[nested]` groups must consume them as-is.
	let config = NestedConfig::new()
		.with_config_file()
		.expect("Failed to fetch config from file");
	...
	let partial = PartialNestedConfig {
		group: PartialNestedGroup {
			port: None,
			list_of_names: Some(vec!["Dana".to_string()]),
		},
		maybe_boo: None,
	};
	let mut config = NestedConfig::new();
	partial.apply_to_config(&mut config);
	assert_eq!(config.group.port, 3000);
	assert_eq!(config.group.list_of_names, vec!["Dana".to_string()]);
}
