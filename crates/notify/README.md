...
| Product-facing test notifications say Coppice; stored channel ids, the `stump-*` ntfy topic prefix, and `X-Stump-Signature` remain stable technical contracts | Renaming display copy must not strand subscriptions or break webhook verifiers.                                                                                        | `channel.rs::Notification::test`; `ntfy.rs::default_topic`; `webhook.rs::SIGNATURE_HEADER` |
| Social recommendation/share/request kinds are direct-recipient, non-administrative events                                                                     | The GraphQL social mutations enqueue only for the exact recipient or sender involved; these events must not fan out through subscriber rules or expose peer identities | `channel.rs::NotificationKind`; `crates/graphql/src/social.rs`                             |
