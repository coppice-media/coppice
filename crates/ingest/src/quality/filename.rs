	...
	async fn run(
		...
		if !enabled_setting(settings) {
			return Ok(outcome(
				self.id(),
				self.name(),
				crate::contract::QualityStatus::NotApplicable,
				0.0,
		...
		let status = match parsed.status {
			FilenameParseStatus::Pass => crate::contract::QualityStatus::Pass,
			FilenameParseStatus::Warn => crate::contract::QualityStatus::Warn,
			FilenameParseStatus::Fail => crate::contract::QualityStatus::Fail,
		};
		let normalized_score = match status {
			crate::contract::QualityStatus::Pass => 1.0,
			crate::contract::QualityStatus::Warn => 0.5,
			crate::contract::QualityStatus::Fail => 0.0,
			crate::contract::QualityStatus::NotApplicable => 0.0,
		};
		Ok(outcome(
			self.id(),
			self.name(),
