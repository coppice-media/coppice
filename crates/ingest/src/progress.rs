...
impl ProgressHub {
	...
	) -> IngestResult<IngestProgressEvent> {
	...
	async fn prune(&self, cursor: i64) -> IngestResult<()> {
	...
	async fn oldest_cursor(&self) -> IngestResult<Option<String>> {
...
mod tests {
	...
	use crate::contract::{AnalysisPhase, DropItemStatus, IngestProgressEvent};
	...
	async fn replay_and_cursor_expiry_are_deterministic() {
		...
		for id in ["one", "two", "three"] {
			ingest_drop_item::ActiveModel {
				...
				drop_group_id: Set(None),
				sidecar_paths: Set(None),
