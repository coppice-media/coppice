...
pub struct IngestCoordinator {
	...
	/// Where notification-routed outcomes go (quality failures, review waits,
	/// provider matches, job failures). `None` in tests.
...
impl IngestCoordinator {
	...
	/// Attach the host's event sink so analysis outcomes are announced (and
	/// routable to notification channels).
	pub fn with_event_sink(mut self, events: Arc<dyn IngestEventSink>) -> Self {
		self.events = Some(events);
		self
	}

	fn notify(&self, event: IngestEvent) {
	...
	pub async fn enqueue_media(
		...
		if media_ids.is_empty() {
			return Err(IngestError::BadRequest("no media selected".to_string()));
		}
	...
	pub async fn pause(&self, job_id: &str) -> IngestResult<AnalysisJobModel> {
		let job = self
			.job(job_id)
			.await?
	...
	pub async fn resume(&self, job_id: &str) -> IngestResult<AnalysisJobModel> {
		let job = self
			.job(job_id)
			.await?
			.ok_or_else(|| IngestError::NotFound(format!("analysis job {job_id}")))?;
	...
	pub async fn cancel(&self, job_id: &str) -> IngestResult<AnalysisJobModel> {
		let job = self
			.job(job_id)
			.await?
			.ok_or_else(|| IngestError::NotFound(format!("analysis job {job_id}")))?;
	...
	pub async fn retry(&self, job_id: &str) -> IngestResult<AnalysisJobModel> {
		let job = self
			.job(job_id)
			.await?
			.ok_or_else(|| IngestError::NotFound(format!("analysis job {job_id}")))?;
		...
		jobs.into_iter().next().ok_or_else(|| {
			IngestError::InternalError("retry did not enqueue a job".to_string())
		})
	}
	...
	pub async fn requeue(
		...
		jobs.pop().ok_or_else(|| {
			IngestError::InternalError("requeue did not enqueue a job".to_string())
		})
	}
	...
	pub async fn job(&self, job_id: &str) -> IngestResult<Option<AnalysisJobModel>> {
	...
	async fn run(&self, job_id: String) -> IngestResult<()> {
		let permit = self
			.semaphore
			.clone()
			.acquire_owned()
		...
		let run_result = match targets.as_slice() {
			[target] if matches!(target, AnalysisTarget::DropItem(_)) => {
				let item = self.store.item(target.id()).await?.ok_or_else(|| {
					IngestError::NotFound(format!("ingest item {}", target.id()))
				})?;
			...
			_ => Err(IngestError::InternalError(format!(
		...
		match run_result {
			...
			Err(error) => {
				...
				self.notify(IngestEvent::AnalysisJobFailed {
	...
	async fn run_phases(
		...
		if !saved_candidates.is_empty() {
			self.notify(IngestEvent::ProviderMatchDone {
				library_id: item.library_id.clone(),
		...
		if updated.status == DropItemStatus::AwaitingReview.as_str() {
			self.notify(IngestEvent::AwaitingReview {
				library_id: item.library_id.clone(),
				drop_item_id: item.id.clone(),
...
mod tests {
	...
	async fn library_rework_runs_quality_checks_against_media_rows() {
		use crate::contract::IngestMetadataProvider;
		use crate::providers::EmbeddedProvider;
		...
		let (source_sha256, byte_size) =
			crate::staging::hash_file(&fixture_path).await.unwrap();
		media::ActiveModel {
		...
		let stump_config = Arc::new(crate::config::IngestSettings::debug());
		let store = crate::store::IngestStore::new(stump_config.clone(), conn.clone());
		let quality = Arc::new(QualityRegistry::builtin(conn.clone()));
		let providers = Arc::new(ProviderRegistry::new(
			conn.clone(),
			Arc::new(crate::host::NoProviderClients),
		));
	...
	async fn preprocess_hook_runs_before_staged_analysis() {
		...
		let (staged_sha, staged_size) =
			crate::staging::hash_file(&staged_path).await.unwrap();
		let (expected_sha, _) = crate::staging::hash_file(&replacement).await.unwrap();
		...
		let mut stump_config = crate::config::IngestSettings::debug();
		stump_config.preprocess_command = Some(hook.to_string_lossy().into_owned());
