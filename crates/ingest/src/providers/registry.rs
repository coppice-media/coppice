...
use models::{
	entity::{ingest_plugin_setting, metadata_provider_config},
...
/// providers.  Remote clients come from the host, which owns the server
/// encryption key needed to decrypt their stored API tokens.
...
fn best_identity(
	mut identities: Vec<crate::contract::ProviderIdentity>,
) -> Option<crate::contract::ProviderIdentity> {
...
mod tests {
	...
	async fn catalog_is_deterministically_sorted_and_has_embedded_provider() {
		...
		let registry =
			ProviderRegistry::new(conn, Arc::new(crate::host::NoProviderClients));
	...
	fn best_identity_requires_confidence_above_half() {
		let identity = crate::contract::ProviderIdentity {
