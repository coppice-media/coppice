...
fn series_fields(metadata: &ExternalSeriesMetadata) -> BTreeMap<MetadataField, Value> {
	...
	insert_strings(&mut fields, MetadataField::Genres, metadata.genres.clone());
	insert_strings(&mut fields, MetadataField::Tags, metadata.tags.clone());
	// `Debug` is the spelling the legacy series apply path writes into
	// `series_metadata.status` (`core/src/filesystem/metadata/apply.rs`), so a
	// candidate and a legacy fetch agree on the value.
	insert_string(
		&mut fields,
		MetadataField::Status,
		metadata.status.map(|status| format!("{status:?}")),
	);
