...
| A challenged probe is `DEGRADED` with `error: "cloudflare challenge"` and clears `consecutive_failures`; a challenge **never** escalates to `DEAD`, and the probe is always sent without configured headers | `DEAD` hides a source from `providerCatalog`, and hiding a live source for a reason an operator fixes with a cookie is wrong. Probing unauthenticated is what makes the row honest: it reports what any client sees, not what one configured instance can reach | `src/health.rs::{CHALLENGE_ERROR,HealthProbe::challenged,next_state,HealthChecker::probe}`; test `a_challenged_source_stays_degraded_and_never_dies` |
...
```text
cargo test -p stump_provider                       # 59 unit tests: browse TTL/ids, materialise, skip/hide/restore reconciliation, cache, GC, catalog, health, dedupe/merge
