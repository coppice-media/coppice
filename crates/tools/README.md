...
| MOBI / KF8                               | MobileRead <https://wiki.mobileread.com/wiki/MOBI>, <https://wiki.mobileread.com/wiki/KF8>                                                                                                       | Read only as the _format_ behind `mobi2epub`; the parsing itself lives in `stump_media::MobiBook` and the citations are on its structures. Neither calibre's, KindleUnpack's nor boko's source is used                                                                                                                                                                                                     |
...
| QuickTime chapter lists                  | <https://developer.apple.com/documentation/quicktime-file-format/chapter_lists>                                                                                                                  | Why `audio-chapters` writes a Nero `chpl` list and never a chapter track — a chapter track is a media _track_, and a player uses the first chapter list it finds, which is also the precedence `stump_media::audio` probes with                                                                                                                                                                            |

## Decisions

| Decision                                                                                                                                                                                                                                                                 | Why                                                                                                                                                                                                                                                                                                                                                                                                                                           | Evidence                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AudioAssembleOptions` implements `Default` by hand instead of deriving it                                                                                                                                                                                               | `ToolInput::parse_options` resolves a _null_ option blob through `Default` and a present object through `serde`, so the two only agree while `Default` agrees with the `#[serde(default = ...)]` attributes. A derived `Default` gave `allow_transcode: false`, and every caller that passed no options at all — the ingest auto-assemble among them — silently refused every MP3 book while the same tool invoked with `{}` assembled it     | `src/audio_assemble.rs` `impl Default for AudioAssembleOptions`; test `no_options_and_an_empty_object_plan_the_same_transcode`                                                                                                                                                                                            |
...
| CBZ output matches the containers Coppice already writes: `ComicInfo.xml` first, `CompressionMethod::Stored`, root-level pages `0001.ext` (1-based, 4-wide)                                                                                                              | One page-naming convention across the codebase; pages are already compressed images                                                                                                                                                                                                                                                                                                                                                           | `src/util.rs` `write_cbz`; `crates/provider/src/host.rs:601`; `crates/media/src/transform/container.rs:58`                                                                                                                                                                                                                |
...
| Generated `ComicInfo.xml` writes the language as `<LanguageISO>`, accepting that it does not round-trip into Coppice's own metadata                                                                                                                                      | `LanguageISO` is the schema element every other reader (Kavita, Komga, ComicRack) uses; `stump_media`'s reader only aliases `<Language>`, and fixing that means editing another crate's metadata contract — flagged rather than silently emitting a non-schema element                                                                                                                                                                        | `src/util.rs` `ComicInfo::to_xml`, `generated_comic_info_parses_as_stump_media_metadata`; `crates/media/src/media/metadata.rs:97-98`                                                                                                                                                                                      |
| `missing-sequence` is report-only: `apply` writes nothing and re-emits the plan's actions as `applied`                                                                                                                                                                   | The finding _is_ the deliverable, and a report tool with a mutating apply would make `plan` a lie; the CLI still gets one uniform `plan`/`apply` shape                                                                                                                                                                                                                                                                                        | `src/missing_sequence.rs` `apply_writes_nothing_and_re_emits_the_findings`                                                                                                                                                                                                                                                |
...
| `epub-check --fix` relays out the whole archive — `mimetype` written first, stored, exact — and raw-copies every other entry, re-serialising only the package document                                                                                                   | The OCF rules are about entry _order_ and _compression_, which an in-place append cannot change; raw copy means a repair provably cannot alter a content document, image, font or style sheet                                                                                                                                                                                                                                                 | `src/epub_check.rs` `repair_epub`, `repaired_epub_is_clean_and_readable`, `compressed_and_padded_mimetype_is_normalized` (asserts `data_start() == 30 + 8`, i.e. no extra field)                                                                                                                                          |
...
| `meta-edit` rewrites `ComicInfo.xml` and EPUB package metadata with one quick-xml event pass, or an M4B/M4A's MP4 item list through `mp4ameta`; archive members and MP4 atoms outside the selected metadata container are raw-copied                                     | Hand-made XML keeps its declaration, comments, unknown elements, attributes and indentation, while a tagger given only `write_meta_items` cannot rewrite AAC samples, artwork or chapters                                                                                                                                                                                                                                                     | `src/meta_edit.rs` `rewrite`/`write_values`/`write_mp4_tags`, `an_edit_keeps_every_unknown_comic_info_byte_and_every_page`, `mp4_plan_apply_preserves_media_and_both_chapters`, `mp4_cover_survives_metadata_edit`                                                                                                        |
| A field's `from` value is read from the element or MP4 tag that stores it, never from a normalised model; `set` only ever sets (an empty value is `ToolError::Options`), and unsupported fields warn without adding an action                                            | A diff has to be about the bytes in the file, so `no-changes` means nothing would move; clearing metadata is a data-loss button nobody asked for, and a plan must show the exact values the apply will write                                                                                                                                                                                                                                  | `src/meta_edit.rs` `Values`/`read_mp4_tags`/`plan_file`, `plan_writes_nothing_and_reports_only_the_changed_fields`, `mp4_unsupported_fields_warn_without_writing`                                                                                                                                                         |
| M4B/M4A edits stage a copy beside the source, preserve its mode after the atomic rename, and leave the source intact on a tagger error; `number`, `volume`, `publisher` and `language` remain unsupported for MP4                                                        | The MP4 tagger rewrites a container in place to patch offsets, so a sibling copy is the only way a parse or write failure can avoid truncating the audiobook; the existing warning shape keeps mixed requests predictable                                                                                                                                                                                                                     | `src/meta_edit.rs` `edit_mp4_atomic`, `mp4_write_failure_keeps_original_bytes`, `mp4_unsupported_fields_warn_without_writing`                                                                                                                                                                                             |
| A rename renders the _post-edit_ values, drops any whitespace chunk holding a value-less placeholder, and claims its target inside the plan                                                                                                                              | `v{volume:02}` must not leave an orphan `v` when a book has no volume, and two books in one run that render to one name must not overwrite each other before any `exists` check could see it                                                                                                                                                                                                                                                  | `src/meta_edit.rs` `Pattern::render`/`plan_file`, `a_rename_from_metadata_round_trips_through_the_scanner_parser`, `a_placeholder_with_no_value_drops_its_whole_chunk`, `two_books_rendering_to_one_name_never_overwrite_each_other`                                                                                      |
...
| Product-facing tool descriptions and parse errors say Coppice; tool ids, `.stump-*` work files, crate names, and option schemas remain stable                                                                                                                            | The display-name cutover must not invalidate plans, manifests, backups, or automation.                                                                                                                                                                                                                                                                                                                                                        | `src/webp_convert.rs::describe`; `src/meta_edit.rs`; `src/lib.rs::Tool`                                                                                                                                                                                                                                                   |
...
| A definition's `version` is the extension's own `versionCode`; the theme's `baseVersionCode` is deliberately not folded in                                                                                                                                               | Upstream adds the two to version an _APK_ built from a Kotlin theme, but here the theme is Rust and ships with Stump — folding it in would bump every definition whenever the upstream counterpart of our own engine changed                                                                                                                                                                                                                  | `src/sources_import.rs` `Definition::version`, `docs/content/docs/developer/source-definitions.mdx` schema table                                                                                                                                                                                                          |
...
| `audio-assemble` demuxes with symphonia and muxes with the `mp4` crate, rather than using one library for both                                                                                                                                                           | Every M4B ffmpeg has ever written with a cover carries the artwork as an `attached_pic` video track, and the `mp4` crate refuses the whole file because it cannot parse a PNG sample entry — which would send a book that is _already_ AAC down the lossy transcode path. symphonia reads the audio stream and ignores the rest, which is exactly the question being asked                                                                    | `src/audio_assemble/mux.rs` `aac_shape`/`remux`; `remux_produces_a_single_track_faststart_m4b` uses `plain.m4b`, which has exactly that cover track                                                                                                                                                                       |
...
| The transcode path stages its full M4B beside the requested output, never in the system temporary directory                                                                                                                                                              | `/tmp` is commonly a small tmpfs; one long book plus the tag/faststart rewrite can exhaust it even when the library filesystem has hundreds of gigabytes free. Sibling staging also removes the cross-filesystem copy the old path required, while `write_atomic` still controls the final rename                                                                                                                                             | `src/audio_assemble.rs` `transcode_parts`; live two-book assembly on a 16 GiB `/tmp` tmpfs                                                                                                                                                                                                                                |
| Assembly derives each permanent chapter boundary from the clock that will join the parts: packet durations for a pure-Rust remux, libavformat's `format.duration` through sibling `ffprobe` for an ffmpeg concat transcode                                               | Symphonia's cheap MP3 fallback can count trailing tags as audio, while a packet sum can discard delay that ffmpeg retains between concat inputs. On the live Dark Eden input the cheap clock moved the last chapter by 220.7 seconds; matching the writer's clock reduced the planned totals to 5 ms of the independent source-duration sums before encode                                                                                    | `src/audio_assemble/mux.rs` `exact_audio_duration_ms`; `src/ffmpeg.rs` `probe_duration_ms`; `src/audio_assemble.rs` `exact_for_assembly`                                                                                                                                                                                  |
| A sidecar chapter list gets one parser per format, chosen by filename, never one permissive parser                                                                                                                                                                       | The three formats disagree about the _unit_ of the number they write — milliseconds, decimal seconds, and CD frames of 1/75 s — and two use indistinguishable `MM:SS:xx` shapes. Guessing the unit from the value is how a chapter list ends up 22% out                                                                                                                                                                                       | `src/audio_assemble/sidecar.rs` `parse_chapters_txt`/`parse_cue`/`parse_audible_json`; `cue_converts_frames_not_hundredths`                                                                                                                                                                                               |
| Every assembled output is probed and rejected if it is not the book the plan described                                                                                                                                                                                   | The three ways an assemble goes wrong silently — a dropped part, a lost chapter list, a non-streamable container — all produce a file that opens and plays. Verification now allows exactly 250 ms (`DURATION_TOLERANCE_MS`), enough for AAC/container rounding but not the minutes of MP3 duration drift the old 1% tolerance hid; a failed verification deletes it                                                                          | `src/audio_assemble.rs` `verify`/`DURATION_TOLERANCE_MS`; `apply` removes the target on error                                                                                                                                                                                                                             |
| `ffmpeg` is the crate's one _encoding_ shell-out, `ffprobe` supplies its concat clock, and `ffmpeg::argv` remains pure and public                                                                                                                                        | MP3/Opus cannot be remuxed into AAC and there is no maintained pure-Rust AAC encoder; the companion probe uses the exact libavformat duration the concat demuxer uses, without decoding. The encoding flags are still asserted in a test on a machine with no ffmpeg at all                                                                                                                                                                   | `src/ffmpeg.rs` `argv`, `encode`, `probe_duration_ms`; `single_input_reads_the_file_directly`, `metadata_and_cover_are_mapped_by_index`, `missing_ffmpeg_names_the_tool_and_the_fix`                                                                                                                                      |
...
is set. The unit is the _file_: a folder book is many containers, each carrying
...
`to: "azw3"` from an EPUB source is the _send-to-Kindle_ path (`KindleExport`):
...
the additions. A single target path makes `front`/`back` a _manual_
assignment; the same option over several archives is the _global_ fallback.
...
Bulk-sets metadata in CBZ `ComicInfo.xml`, EPUB package documents, and M4B/M4A
MP4 tags, then renames files from the values it just wrote. Values are set,
never cleared, and travel as strings so `3.5` and `007` survive;
`volume`/`year` must parse as integers and `number` as a decimal. MP4 actions
identify the `mp4-tags` container and write only iTunes metadata; AAC samples,
cover art, Nero `chpl`, and a QuickTime chapter track survive. A plan lists the
per-field `from`/`to` diff and writes nothing.

| Option                 | Type     | Default                                           | Effect                                                          |
| ---------------------- | -------- | ------------------------------------------------- | --------------------------------------------------------------- |
| `set.series`           | string   | none                                              | `Series` / `calibre:series` / MP4 `©alb`                        |
| `set.number`           | string   | none                                              | `Number` / `calibre:series_index` (decimal); unsupported in MP4 |
| `set.volume`           | string   | none                                              | `Volume` (integer); unsupported in EPUB and MP4                 |
| `set.title`            | string   | none                                              | `Title` / `dc:title` / MP4 `©nam`                               |
| `set.writer`           | string   | none                                              | `Writer` / `dc:creator` / MP4 `©ART`                            |
| `set.narrator`         | string   | none                                              | MP4 `©wrt`/composer; unsupported in CBZ and EPUB                |
| `set.publisher`        | string   | none                                              | `Publisher` / `dc:publisher`; unsupported in MP4                |
| `set.year`             | string   | none                                              | `Year` / `dc:date` / MP4 `©day` (integer)                       |
| `set.language`         | string   | none                                              | `LanguageISO` / `dc:language`; unsupported in MP4               |
| `set.tags`             | string[] | `[]`                                              | `Tags` / one `dc:subject` each / MP4 `©gen` values              |
| `set.description`      | string   | none                                              | MP4 `©des` (`desc` atom); unsupported in CBZ and EPUB           |
...
and a `NOTICE`. Stump runs no Kotlin, so the repository is read as _text_ by a
...
Re-encodes the pages of a CBZ as WebP through Coppice's own page pipeline
...
| `src/meta_edit.rs`              | `meta-edit`: shared plan/apply field model, ComicInfo/OPF rewrites, MP4 `ilst` tags, and sibling-atomic M4B/M4A edits                                                                                                            |
...
| `src/sources_import.rs`         | `sources-import`; the crate's one tool that reads a source _repository_ instead of books — no `ToolInput` paths beyond the checkout root, and the only writer of the definition schema                                           |
...
| `src/audio_chapters.rs`         | `audio-chapters`; the crate's only in-place _container_ edit — `edit_atomic` stages a copy of the original in `util::write_atomic`'s temp file, because a tagger rewrites a container in place                                   |
