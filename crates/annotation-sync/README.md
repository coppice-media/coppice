...
| `liseur-sync:` projections are omitted from native rows, and CAS records with `stump-native:` IDs are omitted from the Liseur lane; each source annotation is exported once with its source color | Native→CAS mirrors must not appear a second time beside their Home row in deterministic user exports | `model.rs` `liseur_annotations`, `build_native_book`; `builds_native_and_liseur_books_in_canonical_order` |
