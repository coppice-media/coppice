			...
			let reading_state_media_ids = reading_session::Entity::find()
				...
				.filter(
					sea_orm::Condition::any()
						...
						.add(reading_session::Column::UpdatedAt.gte(previous_sync_at)),
				)
				.into_tuple::<String>()
				.all(db)
				.await?;
...
impl<'a> SyncPage<'a> {
	...
	pub async fn sync_items(
		...
		for m in items {
			...
			let metadata_changed_since_last_sync =
				self.previous_sync_at.is_some_and(|ps| {
					m.media
						.modified_at
						.is_some_and(|modified_at| modified_at >= ps)
				});
			let reading_state_changed_since_last_sync =
				self.previous_sync_at.is_some_and(|ps| {
					m.reading_session.as_ref().is_some_and(|session| {
						session.updated_at.unwrap_or(session.created_at) >= ps
					})
				});
			...
			if reading_state_changed_since_last_sync {
				if let Some(session) = m.reading_session.as_ref() {
					sync_items.push(SyncItem::ChangedReadingState(
						ReadingStateContainer {
							reading_state: ReadingState::from_active_reading_session(
								m.media.id.clone(),
								session,
							),
						},
...
mod tests {
	...
	#[tokio::test]
	async fn test_represent_changed_reading_state() {
		let db = test_database().await;
		let user = fake_data::User::new("ishmael").insert(&db).await;
		let user = user::AuthUser {
			id: user.id,
		...
		.insert(&db)
		.await;
		let previous_sync = kobo_sync_session::ActiveModel {
			id: Set("previous".to_string()),
			user_id: Set(user.id.clone()),
			media_ids: Set(kobo_sync_session::MediaIds(vec![book.id.clone()])),
			device_id: Set("kobo-1".to_string()),
		...
		let mut previous_sync: kobo_sync_session::ActiveModel = previous_sync.into();
		previous_sync.created_at = Set(previous_sync_at);
		previous_sync
			.update(&db)
			.await
		...
		assert_eq!(vec![book.id.clone()], sync_page.media_ids);
		let sync_items = sync_page
			.sync_items("https://stump.example.org")
			.await
			.expect("failed to retrieve state sync items");
		assert_eq!(sync_items.len(), 1);
		let SyncItem::ChangedReadingState(state) = &sync_items[0] else {
			panic!("expected a ChangedReadingState")
		};
		assert_eq!(state.reading_state.entitlement_id, book.id);
		assert_eq!(
			state
				.reading_state
				.current_bookmark
				.progress_percent
				.expect("state has progress"),
			42.0
		);
	}
