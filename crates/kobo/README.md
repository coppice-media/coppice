...
| Emitted items: `NewEntitlement`, `ChangedProductMetadata`, `ChangedReadingState`, and `ChangedEntitlement` with `IsRemoved` true for a book that left the device's library scope | Nothing else in Stump maps to an entitlement change; a library-scope narrowing does (see the row below) | `src/sync.rs:249-338`; `core/src/kobo/sync_types.rs:24-30` |
| Both directions of a library-scope change are stored in `device_entitlement_deltas` (one row per device + book, `removed` telling them apart, latest transition winning) and drained by the next sync: `removed` rows become `ChangedEntitlement`/`IsRemoved`, the others `NewEntitlement`. Removal metadata is loaded with `VisibilityScope::inherit`; removals ride the final page, and pruning happens in `begin_new_sync` | Neither direction is derivable after the fact: a book that left the scope is invisible to the visibility-filtered query, and a book that entered has no `created_at`/`modified_at` change for the incremental window to notice. Removal items must therefore look the book up outside the device's scope, and a book the user genuinely lost loads nothing and emits nothing | `src/sync.rs`; `crates/devices/src/service.rs::set_library_scope`; `crates/migrations/src/m20260925_000000_add_device_library_scope.rs`; `docs/content/docs/developer/devices.mdx` (Library scope) |
