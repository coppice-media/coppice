...
| Sync is EPUB-only (`extension == "epub"`) | Stock Kobo sync cannot deliver CBZ; WebP pages unsupported anyway | `src/sync.rs:77-120`; test `test_only_includes_epubs` |
...
| KEPUB served from the same `/file/epub` URL, lazily converted and cached by `<media-id>-<mtime>-<options>-d<level>`; `Range` preserved | Kobo discovers the URL during sync and resumes large downloads | `apps/server/src/routers/kobo_backend/kepub.rs:18-24,61-126` |
