...
| --- | --- | --- |
| `RarProcessor` gained `list_entries`/`extract_all` beside the page-processor impl, and a member whose path would escape the destination is skipped rather than sanitised | Staged ingest has to answer "what is inside this rar" without extracting it and then extract a whole tree to disk, neither of which the `FileProcessor`/`FileConverter` traits express — the converter's rar path only ever produced one zip in the cache directory. Reusing the same `unrar` bindings the page lane uses means a rar ingest cannot succeed where a rar *read* would fail. An archive naming `../../etc/passwd` is hostile, and the honest response to a hostile member is to write it nowhere | `src/media/format/rar.rs` `{RarEntry, RarProcessor::list_entries, RarProcessor::extract_all, is_contained}`; consumed by `crates/ingest/src/archive.rs` `{list_rar, extract_rar}` |
