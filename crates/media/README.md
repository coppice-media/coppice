...
| An audiobook is processed by `AudioProcessor` (`media/format/audio.rs`), a `FileProcessor` like every other format | `process()` is the scanner's only entry point, so a shape that cannot be processed cannot be scanned. The page-oriented methods return `FileError::UnsupportedFileType` while `get_page_count` returns `Ok(-1)`: a count is a fact about a book with no pages, a page is not, and a plausible wrong page would hide a routing bug | `src/media/format/audio.rs:37,101-105,173,188,192,199,207` (`AUDIO_PAGES`, the shared `no_pages()` refusal); `cargo test -p stump_media audio` (18 tests) |
...
| `src/media/format/audio.rs` | `AudioProcessor`: the `FileProcessor` for a single audio container or a folder book, its hashed-file rule, and the typed refusals for the page routes |
