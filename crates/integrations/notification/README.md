...
| --- | --- | --- |
| Trait is split into a *static* `payload_from_event` and an async `send_message` | Lets the JSON body be asserted without a network call — the only non-ignored Discord test does exactly that. | `src/lib.rs:15-22`, `src/discord_client.rs:91-105` |
...
| Sender identity is hard-coded (`NOTIFIER_ID = "Stump Notifier"`, `FAVICON_URL`) | Discord webhooks render a per-message username/avatar; one constant keeps every message attributable. | `src/lib.rs:12-13`, `src/discord_client.rs:35-36` |
...
passes on a rejection *or* on no network at all (`src/telegram_client.rs:77-88`). Both
