...
| Reference                                                                | Relation                                                                                                                                                                                      |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Upstream Stump `crates/tests`                                            | Origin of `db::test_database` and the `fake_data` builder style ("`None` means _use some default_"), `src/fake_data.rs:15-17`.                                                                |
...
| The in-memory entity schema includes `runtime_components` and `device_telemetry` | Device/runtime service tests must exercise the same typed tables as production; the fixture still remains entity-only and migration-free | `src/db.rs:1-11,31-80`; `crates/models/src/entity/{runtime_component,device_telemetry}.rs` |
| The in-memory entity schema includes `media_sync_map` | Read-aloud and paired-position tests consume the typed sync-map table without needing the migration graph; omitting it turns valid resolver tests into `no such table` failures. | `src/db.rs`; `crates/models/src/entity/media_sync_map.rs` |
