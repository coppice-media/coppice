...
use stump_media::media::{ProcessedFileHashes, ProcessedMediaMetadata};
...
/// The override options for a scan job. These options are used to override the default behavior,
/// which generally means that the scanner will visit books it otherwise would not. How much extra
/// work is done depends on the specific options.
...
impl ScanOptions {
	...
	/// If the operation cannot be inferred because it depends on context such as the modified time
	/// of the book, this method returns `None`.
