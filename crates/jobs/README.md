...
| Each scheduled row gets its own `cron_loop` task: compute next occurrence → sleep → `dispatch_scheduled` → update `last_run_at`; an exhausted cron stops the loop | Per-row isolation means one bad schedule cannot stall the others; dispatch failures are logged and the loop continues. `reload` aborts and replaces all loops. | `src/scheduler.rs:64-83,166-220` |
