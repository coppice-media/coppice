...
| Host-owned durable maintenance remains an optional, generic scheduler hook; the current core host uses cron dispatch only                                                                                                                            | The scheduler stays job-type agnostic, and inactive private acquisition polling does not keep a maintenance loop or job runtime alive.                                                           | `src/scheduler.rs`; `core/src/{context.rs,job/services.rs}`                                                                                     |
