...
methods) and does **not** own authentication middleware, route mounting or the
concrete backend (`apps/server/src/routers/abs_backend.rs`), podcasts,
collections, or the core event buses the socket lane is fed from.
...
| Collections answer an empty page, not `404`; playlists are real; podcast episode progress answers `404` | The app opens both tabs on every library. A Stump collection spans every media type, so dressing one up as an audiobook collection would be a lie — but a Stump *reading list* is exactly an ordered set of books, which is what a playlist is, so playlists project onto them. An episode id genuinely does not resolve on a book-only profile | `src/routes/libraries.rs` `collections`; `src/routes/playlists.rs`; `src/routes/me.rs` `episode_progress`; `docs/.../abs-compat.mdx` |
...
| `media.ebookFile` is the confirmed EPUB **edition** of the audiobook, resolved per user | Audiobookshelf folds an `.m4b` and an `.epub` in one folder into one item; Stump keeps two `media` rows and records the pair (`liseur_sync_media_links`, `pair_status='confirmed'`). `ebook_editions` is the batched form of `edition_pair::linked_media(.., Confirmed)` and keeps its rule — a `rejected` anchor yields nothing and a `suggested` link never reaches a client. Only `epub` counterparts are reported | `src/model.rs` `AbsEbookFile`; `src/mapper.rs` `ebook_file`; `apps/server/src/routers/abs_backend.rs` `ebook_editions_for`; `models::domain::edition_pair` |
...
| The play-session contract is a field table transcribed from the app's Kotlin data classes, in the test | The app parses with `jacksonObjectMapper()` (`ApiHandler.kt:21,49`) and `readValue` at `ApiHandler.kt:637` has no try/catch: a missing non-nullable reference field or a `null` non-nullable primitive throws and the session is silently lost. The table pins each field to its declaration line so a DTO change that drops one fails here rather than on a phone | `src/routes/tests.rs` `PLAYBACK_SESSION_FIELDS`, `the_play_session_satisfies_the_apps_kotlin_field_table` |
