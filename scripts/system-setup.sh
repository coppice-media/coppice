...
_CHECK_BUN=${CHECK_BUN:=1}
_CHECK_DAV1D=${CHECK_DAV1D:=0}
_FORCE_INSTALL_BUN=${INSTALL_BUN:=0}
_BUN_VERSION=1.4.1
...
install_bun() {
  if ! command -v curl &> /dev/null; then
    log_error "Bun installation requires curl. Install curl, then rerun this script."
  fi

  curl -fsSL https://bun.sh/install | bash -s "bun-v${_BUN_VERSION}"
...
if [ ${_CHECK_BUN} == 1 ]; then
  if ! command -v bun &> /dev/null; then
    if [ ${_FORCE_INSTALL_BUN} == 1 ]; then
      echo "Installing Bun ${_BUN_VERSION}..."
      install_bun
    else
      read -r -p "Bun ${_BUN_VERSION} is required. Install it now? (y/n) " choice
      case $choice in
        y)
          echo "Installing Bun ${_BUN_VERSION}..."
          install_bun
          ;;
        *)
          echo "Skipping Bun installation. Exiting."
          exit 1
          ;;
      esac
    fi
  ...
  if [ "$(bun --version)" != "${_BUN_VERSION}" ]; then
    log_error "Bun ${_BUN_VERSION} is required; found $(bun --version)."
  fi

  echo "Bun ${_BUN_VERSION} requirement met!"
fi
