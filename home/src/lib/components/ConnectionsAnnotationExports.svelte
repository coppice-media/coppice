<script lang="ts">
	...
	/** Draft values per sink. Secrets start empty and remain write-only. */
	...
	const saveSink = createMutation(() => ({
		...
		onError: (error) =>
			toast.error(error instanceof Error ? error.message : 'Unable to save export settings.'),
	...
	const runSync = createMutation(() => ({
		...
		onSuccess: () => {
			void queryClient.invalidateQueries({ queryKey: ['annotation-sinks'] });
			toast.success('Annotation export queued.');
		},
		onError: (error) =>
	...
	function serialize(sinkId: string): Record<string, unknown> {
		...
		return Object.fromEntries(
			(sink?.settings ?? [])
				...
				.map((definition) => [definition.key, parse(draft[definition.key] ?? '', definition.valueType)])
		);
	}
...
<div class="flex flex-col gap-4">
	<div class="flex flex-wrap items-center gap-2">
		<p class="text-sm text-muted-foreground">
			Exports run after annotation changes settle and use only the sinks you enable.
		</p>
		<Button
			size="sm"
			variant="outline"
	...
	{#if sinksQuery.isPending}
		<div class="flex flex-col gap-4" aria-label="Loading annotation export connections">
	...
	{:else if sinksQuery.isError}
		<Alert variant="destructive">
			<AlertTitle>Unable to load export connections</AlertTitle>
			<AlertDescription>
				{sinksQuery.error instanceof Error
					? sinksQuery.error.message
					: 'The server did not return annotation export sinks.'}
			</AlertDescription>
			<Button type="button" size="sm" variant="outline" class="mt-3" onclick={() => sinksQuery.refetch()}>
				Retry
			</Button>
		</Alert>
	...
	{:else}
		{#if status?.pending}
			<Alert>
				...
				<AlertDescription>
					A debounced export is scheduled for your account and will run when annotations settle.
				</AlertDescription>
			</Alert>
		{/if}
		...
		{#each sinks as sink (sink.id)}
			...
			<Card>
								...
								<div class="grid gap-2 text-sm">
									...
									{#if definition.valueType === 'BOOLEAN'}
										<Switch
											...
											onCheckedChange={(next) => setDraft(sink.id, definition.key, next ? 'true' : 'false')}
										/>
									{:else}
										<Input
											{id}
											...
											oninput={(event) => setDraft(sink.id, definition.key, event.currentTarget.value)}
										/>
									{/if}
				...
				<CardFooter class="flex flex-wrap gap-2">
					<Button size="sm" disabled={savingSink === sink.id} onclick={() => save(sink.id, true)}>
						{savingSink === sink.id ? 'Saving…' : state?.enabled ? 'Save settings' : 'Enable'}
					</Button>
					{#if state?.enabled}
						<Button size="sm" variant="outline" disabled={savingSink === sink.id} onclick={() => save(sink.id, false)}>
							Disable
						</Button>
					{/if}
				</CardFooter>
