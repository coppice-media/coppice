<script lang="ts">
	...
	import { labelKind } from '$lib/book/detail';
				...
				{#each books as book (book.mediaId)}
					<a class="group flex items-center gap-3 rounded-lg border p-3 transition-colors hover:bg-muted/50" href={resolve('/(app)/book/[mediaId]', { mediaId: book.mediaId })}>
						...
						<div class="min-w-0 flex-1">
							...
							<div class="mt-1 flex items-center gap-2"><Badge variant="outline" class="text-[10px]">{labelKind(book.kind)}</Badge><span class="text-[10px] text-muted-foreground">{Math.round(book.score * 100)}% match</span></div>
						</div>
