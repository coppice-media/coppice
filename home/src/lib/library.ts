...
export const READING_STATUS_OPTIONS = (Object.keys(READING_STATUS_LABELS) as ReadingStatus[]).map(
	(value) => ({ value, label: READING_STATUS_LABELS[value] }),
)
...
export function bookProgress(book: ConsoleBookRowFragment): BookProgress {
	// The server returns completed readthroughs newest first.
	const latest = book.readHistory[0]
	const percentage =
