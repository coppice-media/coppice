...
SPA is mounted at `/app`; the server redirects `/` there when the legacy web UI
does not own the root route.
...
| Devices, protocol setup, CrossPoint pairing and LAN delivery | `src/routes/(app)/devices/`, reusable catalog/cards in `src/lib/components/{AddClientDialog,ClientPicker,ClientCard,ClientCapabilities,CredentialReveal,CrossPointTargetSetup,CrossPointDeliveryQueue}.svelte`, `src/lib/graphql/{operations,crosspoint}.graphql` |
...
- The editor link appears only for the server owner or a user with
  `MANAGE_LIBRARY`.
