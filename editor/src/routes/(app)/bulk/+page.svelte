...
<div class="flex flex-col gap-6">
	...
	{:else}
		<Card>
			...
			<CardContent class="p-0"><Table stacked><TableHeader><TableRow><TableHead class="w-12"><Checkbox checked={table.getIsAllRowsSelected()} aria-label="Select all loaded items" onCheckedChange={(checked) => table.toggleAllRowsSelected(checked)} /></TableHead><TableHead>Source</TableHead><TableHead>Size</TableHead><TableHead>Status</TableHead><TableHead>Quality</TableHead></TableRow></TableHeader><TableBody>{#each table.getRowModel().rows as row (row.id)}<TableRow data-state={row.getIsSelected() ? 'selected' : undefined}><TableCell><Checkbox checked={row.getIsSelected()} aria-label={`Select ${row.original.filename}`} onCheckedChange={(checked) => row.toggleSelected(checked)} /></TableCell><TableCell class="max-w-[28rem]"><div class="truncate font-medium">{row.original.filename}</div><div class="truncate text-xs text-muted-foreground">{row.original.id}</div></TableCell><TableCell data-label="Size" class="whitespace-nowrap text-muted-foreground">{formatBytes(row.original.sizeBytes)}</TableCell><TableCell data-label="Status"><StatusBadge status={row.original.status} /></TableCell><TableCell data-label="Quality">{row.original.qualityReport?.score ?? '—'}</TableCell></TableRow>{/each}</TableBody></Table></CardContent>
		</Card>
		<div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
			...
			<CardContent class="p-0"><Table stacked><TableHeader><TableRow><TableHead class="w-12"><Checkbox checked={table.getIsAllRowsSelected()} aria-label="Select all loaded items" onCheckedChange={(checked) => table.toggleAllRowsSelected(checked)} /></TableHead><TableHead>Source</TableHead><TableHead>Size</TableHead><TableHead>Status</TableHead><TableHead>Quality</TableHead><TableHead class="text-right">Metadata</TableHead></TableRow></TableHeader><TableBody>{#each table.getRowModel().rows as row (row.id)}<TableRow data-state={row.getIsSelected() ? 'selected' : undefined}><TableCell><Checkbox checked={row.getIsSelected()} aria-label={`Select ${row.original.filename}`} onCheckedChange={(checked) => row.toggleSelected(checked)} /></TableCell><TableCell class="max-w-[28rem]"><div class="truncate font-medium">{row.original.filename}</div><div class="truncate text-xs text-muted-foreground">{row.original.id}</div></TableCell><TableCell data-label="Size" class="whitespace-nowrap text-muted-foreground">{formatBytes(row.original.sizeBytes)}</TableCell><TableCell data-label="Status"><StatusBadge status={row.original.status} /></TableCell><TableCell data-label="Quality">{row.original.qualityReport?.score ?? '—'}</TableCell><TableCell><div class="flex flex-wrap gap-1 @md/table:justify-end"><Button size="sm" variant="outline" onclick={() => openProviderSearch(row.original.id)}>Find metadata</Button></div></TableCell></TableRow>{/each}</TableBody></Table></CardContent>
		</div>
	{/if}
	<Separator />
