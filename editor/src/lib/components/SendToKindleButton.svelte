<script lang="ts">
	...
	import {
		EditorKindleTargetsDocument,
		EditorSendToKindleDocument
	...
	// One shared cache entry for the whole table: every row resolves the
	// operator's Kindle devices from the same request.
	const targetsQuery = createQuery(() => ({
		queryKey: ['kindleTargets'],
		queryFn: () => request(EditorKindleTargetsDocument, {}),
		enabled: browser
	}));
	...
	const send = createMutation(() => ({
		mutationFn: (deviceId: string) => request(EditorSendToKindleDocument, { mediaId, deviceId }),
		onSuccess: ({ sendToKindle }) => {
			toast.success(
				`Sent ${sendToKindle.format.toUpperCase()} (${formatBytes(sendToKindle.bytes)}) to ${sendToKindle.recipient}.`,
	...
	const sideload = createMutation(() => ({
		...
		onSuccess: ({ filename, bytes, converted }) =>
			toast.success(`Saved ${filename} (${formatBytes(bytes)}).`, {
...
{:else if targets.length > 1}
	<Select.Root type="single" onValueChange={(value) => send.mutate(value)}>
		<Select.Trigger
			class="h-6 gap-1 border-0 px-2 text-xs shadow-none"
