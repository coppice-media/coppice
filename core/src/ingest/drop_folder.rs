...
pub async fn list_files(root: &Path) -> CoreResult<Vec<DropFile>> {
	...
	while let Some(directory) = pending.pop() {
		...
		while let Some(entry) = entries.next_entry().await? {
			...
			let filename = relative
				.file_name()
				.ok_or_else(|| {
					CoreError::BadRequest("drop file has no filename".to_string())
				})?
				.to_string_lossy()
				.into_owned();
			files.push(DropFile {
...
pub async fn stats(root: &Path) -> CoreResult<DropFolderStats> {
	...
	let total_bytes = files.iter().try_fold(0_u64, |total, file| {
		total.checked_add(file.byte_size).ok_or_else(|| {
			CoreError::BadRequest("drop-folder size exceeds u64".to_string())
		})
	})?;
	Ok(DropFolderStats {
		pending_files,
