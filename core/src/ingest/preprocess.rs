...
impl PreprocessHook {
	...
	pub fn from_config(config: &StumpConfig) -> CoreResult<Option<Self>> {
		let Some(raw) = config.ingest.ingest_preprocess_command.as_deref() else {
			return Ok(None);
		};
		let command = raw.trim();
		if command.is_empty() {
			return Err(CoreError::InitializationError(
				"ingest_preprocess_command is set but empty; unset it to disable the preprocess hook".to_string(),
			));
		}
		let timeout_secs = config.ingest.ingest_preprocess_timeout_secs;
		if timeout_secs == 0 {
...
pub fn validate_config(config: &StumpConfig) -> CoreResult<()> {
...
fn resolve_program(command: &str) -> CoreResult<PathBuf> {
	let candidate = Path::new(command);
	if candidate.components().count() > 1 || candidate.is_absolute() {
		return if is_executable_file(candidate) {
			Ok(candidate.to_path_buf())
		} else {
			Err(CoreError::InitializationError(format!(
...
mod tests {
	...
	fn config_with(command: Option<&str>, timeout_secs: u64) -> StumpConfig {
		let mut config = StumpConfig::debug();
		config.ingest.ingest_preprocess_command = command.map(str::to_owned);
		config.ingest.ingest_preprocess_timeout_secs = timeout_secs;
		config
	}
	...
	fn unresolvable_command_fails_validation() {
		...
		assert!(
			matches!(&error, CoreError::InitializationError(message) if message.contains("not an executable file")),
		...
		assert!(
			matches!(&error, CoreError::InitializationError(message) if message.contains("set but empty")),
			"unexpected error: {error}"
		);
	}
	...
	fn zero_timeout_is_rejected_when_a_command_is_set() {
		...
		assert!(
			matches!(&error, CoreError::InitializationError(message) if message.contains("greater than zero")),
			"unexpected error: {error}"
		);
	}
