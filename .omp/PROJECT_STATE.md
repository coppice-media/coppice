...
- Branch `headless-modular` @ batch 11 (PAUSED here 2026-09-07 ~02:50 UTC): tier-1 edition pairing (`pair_editions`, `media_chapter_map`, read-time position conversion, console 'Also available as'; m20260946), remote workers (`stump_worker` crate + `stump-worker` binary, `worker_jobs` m20260944, `/api/v2/workers/socket`, `transcode` first job kind with local ffmpeg fallback installed in `routers::mount`, console /workers), `challenge_solve` job kind + `--chrome` browser worker (chromiumoxide, default-off `browser` feature; live: user click solved readcomicsonline.ru, BUT cf_clearance is TLS-fingerprint-bound so replay from reqwest/curl is 403 - browser lane is the next design, docs still promise the cookie recipe: downgrade them), archive drops explode into items (`crates/ingest/src/archive.rs`, m20260945 drop groups, audio items + editor AudioCard/DropGroupStrip/QualityReportPanel, auto-assemble via `audio-assemble`, fixed null-options bug in stump_tools), official ABS app round 2 (37+ routes, JSON 404 for unknown /api on webui builds, download zip, ebook route + `ebookFile` from confirmed pairs, playlists over reading lists, ebooks filter, cover token), editions design page (docs/developer/editions.mdx). Gate 1880+328 tests (server 109 lib + 109 api + worker), both profiles + `rar`, replays 7/7 on 25600 (full,rar binary, migrations through m20260946).
